...
function utf8_char_bytes(str, i)
	...
	if char_byte < 192 then return 1
	elseif char_byte < 224 then return 2
	elseif char_byte < 240 then return 3
	elseif char_byte < 248 then return 4
	elseif char_byte < 252 then return 5
	elseif char_byte < 254 then return 6
	end
	return 1
end
...
local function utf8_to_unicode(str, i)
	byte_count = utf8_char_bytes(str, i)

	local char_byte = str:byte(i)
	local unicode = char_byte
	if byte_count ~= 1 then
		local shift = 2 ^ (8 - byte_count)
		char_byte = char_byte - math.floor(255 / shift) * shift
		unicode = char_byte * 64 ^ (byte_count - 1)
	end
	for j = 2, byte_count do
		char_byte = str:byte(i + j - 1) - 128
		unicode = unicode + char_byte * 64 ^ (byte_count - j)
	end
	unicode = round(unicode)

	return byte_count, unicode
end

---Get width/height ratio of Unicode code point
---@param unicode integer
---@return number
local function unicode_width(unicode)
	-- could do binary search
	for _, unicode_block in ipairs(unicode_blocks) do
		if unicode >= unicode_block[1][1] and unicode <= unicode_block[1][2] then
			--TODO: width estimation based on unicode block
			break
		end
	end
	return 1
...
---@param str string
---@return fun(): integer, integer, number
function utf8_iter(str)
	local byte_start = 1

	return function()
		if #str < byte_start then return nil end
		local byte_count, unicode = utf8_to_unicode(str, byte_start)
		local start = byte_start
		byte_start = byte_start + byte_count

		local width = unicode_width(unicode)

		local simple_width_estimation = (byte_count > 2 and 2 or 1)
		return start, byte_count, simple_width_estimation
	end
end

---Wraps text at the closest wrapping oportunity to target_line_length
---@param text string
